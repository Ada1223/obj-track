...
video = cv2.VideoCapture('/usr/project/obj-track/2.mp4')
#定义保存视频的参数
   # 设置需要保存视频的格式“MP4” 
...
while True:  
    ...
    if len(cnts1) > 0:
        ...
        rotatedRect1 = cv2.minAreaRect(c1)  # 计算最小外接矩形
    ...
    for i in range(1, len(pts1)):
        ...
        #存储center中心点的值
        f = open('box1.csv', 'w', encoding='utf-8', newline="")
    ...
    if len(cnts2) > 0:
        ...
        rotatedRect2 = cv2.minAreaRect(c2)  # 计算最小外接矩形
    ...
    for i in range(1, len(pts2)):
        ...
        # 存储center中心点的值
        f = open('box2.csv', 'w', encoding='utf-8', newline="")
